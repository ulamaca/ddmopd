...
def amino_based_filtering_relaxed_v1(dataset: pd.DataFrame, seq_col='sequence') -> pd.DataFrame:    
    dataset['pass_f1'] = dataset[seq_col].apply(check_sequence_for_positive_clusters)
    dataset['pass_f2'] = dataset[seq_col].apply(check_sequence_for_hydrophobic_clusters)
    return dataset